let ele = [10,11,12,13,15];
let ele2 = [10, 11];
let ele3 = ele.filter(item => !ele2.includes(item));
                ...
                if(time != undefined) {
                    ...
                    confirmseets.forEach(seetno => {
                        seetno.seetno.forEach(seetconfirm => {
                            confirmseet.push(seetconfirm);
                        });
                    });

                    const conductorConfirm = await conductorConfirmation.find({conductorId: id, timeId: timeId});
                    console.log("conductor Seets " + yconductorConfirm);

                    if(req.session.username == undefined) {
                        res.render('mybus', { allSeatNumbers: allSeatNumbers, confirmseet : confirmseet, busId: busId, timeId: timeId, username: ""});
                    }
                    else {
                        res.render('mybus', { allSeatNumbers: allSeatNumbers, confirmseet : confirmseet, busId: busId, timeId: timeId, username: req.session.username});
                    }
                } else {
